...
class DialPad extends Component {
	...
	searchForThePhoneNumberInDatabase = async () => {
		try {
			...
			//coupon Details key to be remove once, it all other components are ported from couponDetails
			// to userInfo and Coupons

			this.props.updateCoupons({
				couponDetails: this.couponsDetails,
				allCoupons: couponDetailsArray,
				userInfo: response.data.response.response.Customer[0]
			});
...
const mapStateToProps = (state) => {
	return {
		...
		userInfo: state.UserIdentification.userInfo,
		allCoupons: state.UserIdentification.allCoupons
	};
};
